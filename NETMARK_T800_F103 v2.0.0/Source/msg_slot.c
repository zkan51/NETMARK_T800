...
#define SendInterval		100 //发射间隔系数 发送间隔=(SendInterval+随机系数)*10ms
/************************* *********************************
 * ???:msg_crc
 * ??  : ??????crc?????
 * ??  :????????ptr,????num
...
void send_on(void)
{
		...
		if(flag_gps_data_available){ // 初始化阶段尚未搜索到有效GPS报文时，不发射任何数据
			...
			sendrandom=rand()%50; //产生随机数1-50 ,防止AIS消息碰撞
			//Delay 1s左右 
			SendDelay = SendInterval + sendrandom;
			while(SendDelay)
			{
				delay_ms(10);
				SendDelay--;
