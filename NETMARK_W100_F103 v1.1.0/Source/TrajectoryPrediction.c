...
/*	
	if(mybuf!=NULL){
		idxcomma2 = 0;
		idx = 0;
		for(idxcomma2 = 0; mybuf[idxcomma2]!='\0'; idxcomma2++){
			if(mybuf[idxcomma2]==','){
				idx++;
			}
			if(idx==2){
...
*/
		...
		if(is_gps_data_init && idx_tail<N-1 && gps_data_init_cnt>20){ //已保存的点数不超过N-1,且至少已经获得gps_data_init_cnt个数据时，新进来的数据点无条件保留
			is_data_valid = 1;
